...
class Classifier(object):
    ...
    def predict(self, x:float,y:float,head:float,v:float,delta:float) -> np.ndarray:
        np_data = np.array([x,y,head,v,delta]).astype(np.float32)
        prediction = self.classifier.run(None, {self.input_name: np_data})
        print(prediction)
        output = np.array(list(prediction.values()))
...
classifier = Classifier(
    model_filepath="models/kinematic_model_test.onnx",
